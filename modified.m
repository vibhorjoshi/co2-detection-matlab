...
% This loads your saved variables (cube, ctmfScore, d, wavelengths, etc.)
load('proposed_results.mat'); 

disp('2. Calculating Baseline CTMF for comparison...');
% Isolate the SWIR bands from the RAW, unconditioned cube
swirMask = wavelengths >= 1500 & wavelengths <= 2200;
rawSWIR = cube(:, :, swirMask);

% Run standard CTMF on the raw data (K=4, lambda=1e-6, seed=1)
baselineScore = compute_ctmf(rawSWIR, d, 4, 1e-6, 1);

disp('3. Generating P95 thresholds and masks...');
% Match the variable names the plotting script expects
ctmfScore_p = ctmfScore;

% Find the threshold for the top 5% of scores
p95_b = prctile(baselineScore(:), 95);
p95_p = prctile(ctmfScore_p(:), 95);

% Create the binary masks (1 if in top 5%, 0 otherwise)
...
% KS-type max separation on a common grid
grid_pts = linspace(min([bs; cp]), max([bs; cp]), 4000);

% FIX: Handle duplicate values by keeping only the 'last' unique occurrence
[bs_unq, bs_idx] = unique(bs, 'last');
[cp_unq, cp_idx] = unique(cp, 'last');

% Interpolate to find CDF differences using the unique arrays
...
% Construct a custom Red-White-Blue divergent colormap
...
for i = 1:2
    ...
    if n_comp > 0
        ...
        dispImg(mask) = 1; % minor components
        dispImg(stats(maxIdx).PixelIdxList) = 2; % largest component
    else
        maxArea = 0; comp = 0;
    end
