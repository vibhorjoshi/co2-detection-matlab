...
│   ├── fig_stagewise6panel.png    # 6-panel progression visualization
│   ├── fig_ablation2x2.png        # 2x2 algorithm comparison
│   ├── fig_profile_clean.png      # Horizontal row profile comparison
│   ├── fig_hotspot_evolution.png  # Coverage evolution chart
│   ├── threshold_sensitivity.png  # Otsu & Percentile robustness chart
│   ├── fig_connected.png          # Connected components morphology
│   ├── fig_diffmap.png            # Proposed vs Baseline difference map
│   ├── fig_cdf.png                # Cumulative distribution function plots
│   ├── fig_3dsurface.png          # 3D matched-filter response surfaces
│   ├── fig_geospatial_overlay.png # Satellite mapping from Mapping Toolbox
│   └── Figure_1.png               # Enlarged geographic projection view
│
├── Core_Algorithms/               # (Function files for spectral conditioning)
│   ├── compute_cibr.m             # CIBR anomaly detection function
│   ├── compute_jrge.m             # JRGE continuum removal function
│   ├── compute_sfa.m              # SFA template matching function
│   └── compute_ctmf.m             # CTMF final matched filter function
│
├── Helper_Functions/              # (Utility functions for the pipeline)
│   ├── build_target_spectrum.m    # Generates the dual-Gaussian CO₂ template
│   └── get_rgb_composite.m        # Extracts the false-color SWIR context image
│
├── Execution_Scripts/             # (Runnable scripts for the framework)
│   ├── save_proposed_results.m    # Master script: runs pipeline & saves data
│   ├── generate_figures.m         # Generates quantitative validation plots
│   ├── generate_map.m             # Generates geographic projection & satellite map
│   ├── analyse_hotspots.m         # Connected components analysis script
│   └── threshold_analysis.m       # Threshold robustness evaluation script
│
├── generated_data/                # (Created automatically after running master script)
│   └── proposed_results.mat       # Saved workspace variables and binary masks
│
